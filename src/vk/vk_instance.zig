...
pub const vk_core_version = vk.API_VERSION_1_2;
...
var g_instance_dispatch: ?*vk.InstanceWrapper = undefined;
var allocator: std.mem.Allocator = undefined;
pub var vk_alloc: ?*vk.AllocationCallbacks = null;

pub fn init(alloc: std.mem.Allocator, options: InstanceOptions) !vk.InstanceProxy {
    ...
    const vk_app_info: vk.ApplicationInfo = .{
        ...
        .api_version = @bitCast(vk_core_version),
    ...
    var vkd = vk.BaseWrapper.load(vkGetInstanceProcAddr);
    ...
    const instance_dispatch = try alloc.create(vk.InstanceWrapper);
    errdefer alloc.destroy(instance_dispatch);
    g_instance_dispatch = instance_dispatch;
    instance_dispatch.* = vk.InstanceWrapper.load(instance_handle, vkGetInstanceProcAddr); // create wrapper that loads all vk instance functions
    const vki = vk.InstanceProxy.init(instance_handle, instance_dispatch); // ease of use struct that wraps instance_handle together with instance_dispatch so that instance_ptr doesn't need to be passed in every fn
    return vki;
}

pub fn deinit(vk_instance: vk.InstanceProxy) void {
